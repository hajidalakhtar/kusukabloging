				...
				<div class="row">
					<div class="col-md-5 wrapthumbnail">
						<a href="{{Route('details',[$blog->blog->id , $blog->blog->slug])}}">
							<div class="thumbnail" style="background-image:url('{{asset('/image/'.$blog->blog->thumbnail)}}');">
							</div>
						</a>
					</div>
					<div class="col-md-7">
						<div class="card-block">
							<h2 class="card-title pt-3"><a
									href="{{Route('details',[$blog->blog->id , $blog->blog->slug])}}">{{$blog->blog->title}}</a></h2>
								...
								<div class="wrapfooter">
									...
									<span class="author-meta">
										<span class="post-name"><a
												href="{{Route('myprofile',[$blog->blog->provider_id(),$blog->blog->author_id])}}">{{$blog->blog->author}}</a></span><br />
										<span class="post-date">
											{{$blog->blog->created_at->day}}-{{$blog->blog->created_at->month}}-{{$blog->blog->created_at->year}}</span>
									</span>
