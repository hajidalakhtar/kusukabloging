			...
			<div class="row">
				...
				<div class="col-md-7">
					<div class="card-block">
						<h2 class="card-title pt-3" style="overflow: hidden; text-overflow: ellipsis; max-height: 3ch; text-decoration:none; white-space: nowrap;  width: 90%; "><a href="{{Route('details',[$blog->id , $blog->slug])}}"  >{{$blog->title}}</a></h2>
					<h4 class="card-text" style="overflow: hidden; text-overflow: ellipsis; max-height: 11.9ch; text-decoration:none ; "><p >
					<?php
								...
								<a href="{{Route('myprofile',[$blog->provider_id(),$blog->author_id])}}">
                                    ...
                                    <img class="author-thumb" src="{{$blog->poto_profile()}}" alt="Sal">
                                     @endif
															...
															<span class="post-read-more pt-2"><a href="{{Route('deletefavorite',$blog->id)}}" title="Read Story">{{$blog->category}}</a></span>
